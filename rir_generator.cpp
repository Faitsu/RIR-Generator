...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if((rr[0] >= 0 && rr[1] >= 0 && rr[2] >= 0) && (rr[0] <= LL[0] && rr[1] <= LL[1] && rr[2] <= LL[2])){//if the microphone is within the room
        ...
        if(nMicrophones > 1){
         mexErrMsgTxt("Error: There is currently no functionality for calculating more than one microphone" 
                      "outside a room at the same time. Please run this code with only one set of receiver coordinates");}
